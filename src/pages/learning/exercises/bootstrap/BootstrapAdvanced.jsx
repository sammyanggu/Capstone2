...
export default function BootstrapAdvanced() {
    ...
    useEffect(() => {
        const savedStatus = localStorage.getItem('bootstrapAdvancedStatus');
        if (savedStatus) {
    ...
    useEffect(() => {
        localStorage.setItem('bootstrapAdvancedStatus', JSON.stringify(exerciseStatus));
    }, [exerciseStatus]);
...
<body class="bg-light p-5">
    ...
    const handleCodeSubmit = () => {
        ...
        if (correct) {
            ...
            if (currentUser?.uid) {
                completeExercise(currentUser.uid, 'bootstrap', 'advanced', 10);
            }
