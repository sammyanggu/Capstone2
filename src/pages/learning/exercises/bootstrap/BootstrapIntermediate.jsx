...
export default function BootstrapIntermediate() {
    ...
    useEffect(() => {
        const savedStatus = localStorage.getItem('bootstrapIntermediateStatus');
        if (savedStatus) {
    ...
    useEffect(() => {
        localStorage.setItem('bootstrapIntermediateStatus', JSON.stringify(exerciseStatus));
    }, [exerciseStatus]);
...
<body class="p-5">
    ...
    const handleCodeSubmit = () => {
        ...
        if (correct) {
            ...
            if (currentUser?.uid) {
                completeExercise(currentUser.uid, 'bootstrap', 'intermediate', 10);
            }
