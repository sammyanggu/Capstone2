        ...
        const loadExerciseIndex = async () => {
            if (currentUser?.uid) {
                try {
                    ...
                    for (let i = 0; i < 4; i++) {
                        const progress = await getExerciseProgress(currentUser.uid, 'bootstrap', `beginner-${i}`);
                        if (progress && progress.isCompleted) {
                            newStatus[i] = true;
                        }
                    }
                    setExerciseStatus(newStatus);
                    console.log('Loaded Bootstrap beginner exercise statuses:', newStatus);
        ...
        if (correct) {
            ...
            setTimeout(async () => {
                ...
                if (currentExercise < exercises.length - 1) {
                    ...
                    if (currentUser?.uid) {
                        try {
                            await saveCurrentExerciseIndex(currentUser.uid, 'bootstrap', 'beginner', nextIndex);
                            console.log(`✅ Saved Bootstrap beginner index: ${nextIndex}`);
                        } catch (err) {
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                ...
                                <div className="lg:col-span-1">
                                    <CodeFeedback 
                                        ...
                                        exerciseId={`bootstrap-beginner-${currentExercise}`}
                                        level="beginner"
                                    />
                                </div>
                            </div>
